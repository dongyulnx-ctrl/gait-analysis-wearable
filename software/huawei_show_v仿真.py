...
# import obs  # 【修改】暂时注释掉 OBS 的导入，因为本地测试不需要连云
...
# ================= 【修改】本地文件测试配置区 =================
# 把这里改成你要测试的 CSV 文件路径（格式必须是和之前程序保存的一样的最终CSV）
BASE_DIR = _BASE_DIR
CSV_FILE = os.path.join(BASE_DIR, "示例数据", "2026年5月13日正常步态视频测试.csv")
...
def find_raw_text(obj):
    if isinstance(obj, str):
        return obj.strip() if obj.strip().upper().startswith("AA") else None
...
def parse_data(raw_str, last_timestamp):
    ...
    for i in range(0, len(parts), 23):
        group = parts[i:i + 23]
        if len(group) < 23: continue
        try:
            row = []
            for idx, val in enumerate(group):
                num = float(val)
                if idx in [1, 2, 3, 4, 5, 6, 10, 11, 12, 13, 14, 15]:
                    row.append(num / 1000.0)
                elif idx in [7, 8, 9, 16, 17, 18]:
                    row.append(num / 100.0)
                else:
                    row.append(num)
            ts = row[0]
            if cur_last is not None and ts < cur_last - 100:
                continue
            else:
                cur_last = ts
            parsed.append(row)
...
# ================= 【新增】本地文件数据回放线程 =================
class LocalFileWorkerThread(QThread):
    log_signal = pyqtSignal(str)

    def __init__(self, csv_file_path, data_queue, emg_rms_queue, imu1_angle_queue, imu2_angle_queue, imu1_predict_queue,
                 pressure_queue):
        super().__init__()
        ...
        self.imu2_angle_queue = imu2_angle_queue
    ...
    def run(self):
        if not os.path.exists(self.csv_file_path):
            self.log_signal.emit(f"[❌ 找不到本地测试文件: {self.csv_file_path}]")
            return

        self.log_signal.emit(f"[*] 正在读取本地文件: {self.csv_file_path}")
        try:
            # 使用 pandas 直接读取 CSV
            import pandas as pd
            df = pd.read_csv(self.csv_file_path)
            rows = df.values.tolist()
            self.log_signal.emit(f"[*] 成功加载 {len(rows)} 行数据，开始模拟实时推送...")
            ...
            for row in rows:
                ...
                ts = row[0]
                # 根据CSV里真实的时间戳差值来 sleep，模拟真实流速
                if prev_ts is not None:
                    diff_ms = ts - prev_ts
                    # 限制一下极值，防止没数据时瞬间拉完，或者断点时卡死
                    sleep_time = max(0.01, min(diff_ms / 1000.0, 1.0))
                    time.sleep(sleep_time)
                prev_ts = ts

                # 按照原来 OBS 线程的逻辑，把数据分发到各个队列
                try:
            ...
            self.log_signal.emit("[*] 本地文件数据已全部播放完毕！")
        except Exception as e:
            self.log_signal.emit(f"[❌ 读取本地文件失败: {e}]")

    def stop(self):
        self.is_running = False


# ================= OBS 后台下载线程 (原代码保留，但不调用) =================
class OBSWorkerThread(QThread):
    log_signal = pyqtSignal(str)

    def __init__(self, data_queue, emg_rms_queue, imu1_angle_queue, imu2_angle_queue, imu1_predict_queue,
                 pressure_queue):
        super().__init__()
        self.data_queue = data_queue
        self.emg_rms_queue = emg_rms_queue
        ...
        self.emg1_history = deque(maxlen=10)
        self.emg2_history = deque(maxlen=10)
    ...
    def run(self):
        ...
        while self.is_running:
            while not self.imu_queue.empty():
                try:
                    self.buf.append(self.imu_queue.get_nowait())
                except:
                    break
            if len(self.buf) >= 246:
...
class MedicalCardWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        medical_content = [("姓名：", "张三"), ("性别：", "男"), ("年龄：", "60岁"), ("入院时间：", "2026.3.8"),
                           ("主治医生：", "李医生"), ("时间：", self.get_current_time())]
        for i, (label, value) in enumerate(medical_content):
            ...
            label_widget.setStyleSheet("color: #fff; font-weight: bold;font-size: 30px;")
            value_widget = QtWidgets.QLabel(value);
            value_widget.setStyleSheet("color: #fff;font-size: 30px;")
...
class GaitTimeSeriesWidget(QtWidgets.QWidget):
    def __init__(self, shared_results_deque):
        super().__init__();
        self.results_buf = shared_results_deque;
        self.initUI()
    ...
    def initUI(self):
        ...
        self.pw = pg.PlotWidget(title="分类结果时序图");
        self.pw.showGrid(x=True, y=True, alpha=0.2);
        self.pw.setYRange(0.5, 5.5);
        self.pw.setXRange(0, 60)
        custom_ticks = [(1, "正常步态"), (2, "剪刀步态"), (3, "拖曳步态"), (4, "冻结步态"), (5, "慌张步态")]
        self.pw.getAxis('left').setTicks([custom_ticks]);
        layout.addWidget(self.pw)

    def update_plot(self):
        if not self.results_buf: return
        now = time.time();
        times = [t for t, _ in self.results_buf];
        res = [r for _, r in self.results_buf]
        rel_times = [now - t for t in times];
        filtered = [(rt, r) for rt, r in zip(rel_times, res) if rt <= 60]
...
class GaitBarWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        layout.setSpacing(2)
        pg.setConfigOptions(background='#2b2b3c', foreground='w', antialias=True)
        self.pw = pg.PlotWidget(title="病情百分比统计");
        self.pw.showGrid(x=True, y=False, alpha=0.2);
        self.pw.setYRange(0, 100);
        self.pw.setXRange(0, 6)
        bar_ticks = [(1, "正常步态"), (2, "剪刀步态"), (3, "拖曳步态"), (4, "冻结步态"), (5, "慌张步态")]
        self.pw.getAxis('bottom').setTicks([bar_ticks]);
        layout.addWidget(self.pw)

    def update_plot(self):
        if not self.results_buf: return
        now = time.time();
        recent_res = [r for t, r in self.results_buf if (now - t) <= 60]
        ...
        y = [percentages.get(i, 0) for i in x]
        color_map = {1: '#FF6B6B', 2: '#4ECDC4', 3: '#45B7D1', 4: '#FF9F1C', 5: '#9B59B6'}
        brushes = [pg.mkBrush(color_map.get(i, '#FFFFFF')) for i in x]
        bg = pg.BarGraphItem(x=x, height=y, width=0.6, brushes=brushes);
        self.pw.addItem(bg)
...
class MedicineHistoryWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        for name, medicine, date in [("李一", "康复阶段", "2026.4.10"), ("李二", "治疗阶段", "2026.3.3")]:
            ...
            for t in [name, medicine, date]: l = QtWidgets.QLabel(t); l.setStyleSheet(
                "color: #fff;bold;font-size: 30px;"); rl.addWidget(l)
...
class PressurePlotWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.curve_adc1 = self.pw.plot(pen=pg.mkPen('#FFD700', width=2), name="ADC1", clipToView=True,
                                       autoDownsample=True)
        self.curve_adc2 = self.pw.plot(pen=pg.mkPen('#FF4500', width=2), name="ADC2", clipToView=True,
                                       autoDownsample=True)
        layout.addWidget(self.pw)
    ...
    def _tick(self):
        if self.pressure_queue.qsize() > 100:
            for _ in range(self.pressure_queue.qsize() - 50):
                try:
                    self.pressure_queue.get_nowait()
                except:
                    break
        got_data = False
        for _ in range(1):
            try:
                ts, adc1, adc2 = self.pressure_queue.get_nowait(); self.buf_time.append(ts); self.buf_adc1.append(
                    adc1); self.buf_adc2.append(adc2); got_data = True
            except:
                break
        if got_data: self.curve_adc1.setData(self.buf_time, self.buf_adc1); self.curve_adc2.setData(self.buf_time,
                                                                                                    self.buf_adc2)
...
class EMGRmsPlotWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.curve_emg1 = self.pw_emg.plot(pen=pg.mkPen('#00ff88', width=2), name="EMG1_RMS", clipToView=True,
                                           autoDownsample=True)
        self.curve_emg2 = self.pw_emg.plot(pen=pg.mkPen('#FF9F1C', width=2), name="EMG2_RMS", clipToView=True,
                                           autoDownsample=True)
    ...
    def _tick(self):
        if self.emg_rms_queue.qsize() > 100:
            for _ in range(self.emg_rms_queue.qsize() - 50):
                try:
                    self.emg_rms_queue.get_nowait()
                except:
                    break
        got_data = False
        for _ in range(1):
            try:
                ts, rms1, rms2 = self.emg_rms_queue.get_nowait(); self.buf_time.append(ts); self.buf_emg1_rms.append(
                    rms1); self.buf_emg2_rms.append(rms2); got_data = True
            except:
                break
        if got_data: self.curve_emg1.setData(self.buf_time, self.buf_emg1_rms); self.curve_emg2.setData(self.buf_time,
                                                                                                        self.buf_emg2_rms)
...
class IMU1AccGyroWidget(QtWidgets.QWidget):
    def __init__(self, data_queue):
        ...
        self.max_points = 500
        self.buf_time = deque(maxlen=self.max_points);
        self.buf_ax = deque(maxlen=self.max_points);
        self.buf_ay = deque(maxlen=self.max_points);
        self.buf_az = deque(maxlen=self.max_points)
        self.buf_gx = deque(maxlen=self.max_points);
        self.buf_gy = deque(maxlen=self.max_points);
        self.buf_gz = deque(maxlen=self.max_points)
        self.initUI();
        self.timer = QtCore.QTimer(self);
        self.timer.timeout.connect(self._tick);
        self.timer.start(30)
    ...
    def initUI(self):
        ...
        self.pw_acc.showGrid(x=True, y=True, alpha=0.3);
        self.pw_acc.setYRange(-2, 2)
        ...
        self.pw_gyro.showGrid(x=True, y=True, alpha=0.3);
        self.pw_gyro.setYRange(-8, 8)
        self.curve_gx = self.pw_gyro.plot(pen=pg.mkPen('y', width=2), name="Gyro_X", clipToView=True,
                                          autoDownsample=True)
        self.curve_gy = self.pw_gyro.plot(pen=pg.mkPen('c', width=2), name="Gyro_Y", clipToView=True,
                                          autoDownsample=True)
        self.curve_gz = self.pw_gyro.plot(pen=pg.mkPen('m', width=2), name="Gyro_Z", clipToView=True,
                                          autoDownsample=True)
        layout.addWidget(self.pw_acc, stretch=1);
        layout.addWidget(self.pw_gyro, stretch=1)
    ...
    def _tick(self):
        ...
        for _ in range(1):
            try:
                row = self.data_queue.get_nowait(); self.buf_time.append(row[0]); self.buf_ax.append(
                    row[1]); self.buf_ay.append(row[2]); self.buf_az.append(row[3]); self.buf_gx.append(
                    row[4]); self.buf_gy.append(row[5]); self.buf_gz.append(row[6]); got_data = True
            except:
                break
        if got_data: self.curve_ax.setData(self.buf_time, self.buf_ax); self.curve_ay.setData(self.buf_time,
                                                                                              self.buf_ay); self.curve_az.setData(
            self.buf_time, self.buf_az); self.curve_gx.setData(self.buf_time, self.buf_gx); self.curve_gy.setData(
            self.buf_time, self.buf_gy); self.curve_gz.setData(self.buf_time, self.buf_gz)
...
class IMU1AnglePlotWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.curve_az = self.pw.plot(pen=pg.mkPen('#00bfff', width=2), name="Angle_Z", clipToView=True,
                                     autoDownsample=True)
        layout.addWidget(self.pw)
    ...
    def _tick(self):
        if self.imu1_angle_queue.qsize() > 100:
            for _ in range(self.imu1_angle_queue.qsize() - 50):
                try:
                    self.imu1_angle_queue.get_nowait()
                except:
                    break
        got_data = False
        for _ in range(1):
            try:
                ts, ax, ay, az = self.imu1_angle_queue.get_nowait(); self.buf_time.append(ts); self.buf_ax.append(
                    ax); self.buf_ay.append(ay); self.buf_az.append(az); got_data = True
            except:
                break
        if got_data: self.curve_ax.setData(self.buf_time, self.buf_ax); self.curve_ay.setData(self.buf_time,
                                                                                              self.buf_ay); self.curve_az.setData(
            self.buf_time, self.buf_az)
...
class IMU2AnglePlotWidget(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        self.curve_ax = self.pw.plot(pen=pg.mkPen('r', width=2), name="Angle_X", clipToView=True, autoDownsample=True)
        self.curve_ay = self.pw.plot(pen=pg.mkPen('g', width=2), name="Angle_Y", clipToView=True, autoDownsample=True)
        self.curve_az = self.pw.plot(pen=pg.mkPen('#FF69B4', width=2), name="Angle_Z", clipToView=True,
                                     autoDownsample=True)
        layout.addWidget(self.pw)
    ...
    def _tick(self):
        if self.imu2_angle_queue.qsize() > 100:
            for _ in range(self.imu2_angle_queue.qsize() - 50):
                try:
                    self.imu2_angle_queue.get_nowait()
                except:
                    break
        got_data = False
        for _ in range(1):
            try:
                ts, ax, ay, az = self.imu2_angle_queue.get_nowait(); self.buf_time.append(ts); self.buf_ax.append(
                    ax); self.buf_ay.append(ay); self.buf_az.append(az); got_data = True
            except:
                break
        if got_data: self.curve_ax.setData(self.buf_time, self.buf_ax); self.curve_ay.setData(self.buf_time,
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.imu2_angle_queue = Queue(maxsize=5000)
    ...
    def load_ai_model(self):
        ...
        try:
            ...
            self.model = cnn_transformer(output_dim=5, len_channel=6, input_dim=32, hidden_dim=256, num_heads=2,
                                         dropout=0.2).to(device)
            self.model.load_state_dict(
                torch.load(os.path.join(MODEL_DIR, 'best_gait_model_5cls.pth'), map_location=device,
                           weights_only=False))
            self.model.eval()
            print("[*] AI 预测模型加载成功！")
        except Exception as e:
    ...
    def initUI(self):
        self.setWindowTitle("实时步态分类 - 本地文件回放测试版")
        ...
        self.statusBar().showMessage("状态：未运行 (本地回放模式)")

    def create_left_widget(self):
        widget = QtWidgets.QWidget();
        widget.setStyleSheet("QWidget { background-color: rgba(30, 30, 47, 0.8); border-radius: 8px; }")
    ...
    def create_section(self, title, content_widget):
        widget = QtWidgets.QWidget();
        widget.setStyleSheet("QWidget { background-color: rgba(43, 43, 60, 0.8); border-radius: 6px; }")
        layout = QtWidgets.QVBoxLayout(widget);
        layout.setContentsMargins(0, 0, 0, 0);
        layout.setSpacing(0)
        if title:
            tw = QtWidgets.QWidget();
            tw.setFixedHeight(30);
            tw.setStyleSheet(
                "QWidget { background-color: rgba(0, 0, 0, 0.3); border-top-left-radius: 6px; border-top-right-radius: 6px; }")
            tl = QtWidgets.QHBoxLayout(tw);
            tl.setContentsMargins(10, 0, 10, 0)
            tlbl = QtWidgets.QLabel(title);
            tlbl.setStyleSheet("color: #fff; font-weight: bold;font-size: 25px;");
            tl.addWidget(tlbl);
            tl.addStretch()
            layout.addWidget(tw)
    ...
    def create_control_widget(self):
        ...
        layout.setContentsMargins(10, 10, 10, 10)
        self.run_button = QtWidgets.QPushButton("开始实时接受")
        self.run_button.setStyleSheet(
            "QPushButton { background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #667eea, stop:1 #764ba2); color: white; border: 1px solid rgba(255, 255, 255, 0.3); border-radius: 5px; padding: 10px 20px; font-weight: bold; min-height: 20px; } QPushButton:hover { background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #764ba2, stop:1 #667eea); }")
    ...
    def toggle_running(self):
        if not self.running:
            # 【修改】实例化本地文件读取线程，替代 OBS 线程
            self.obs_thread = LocalFileWorkerThread(
                CSV_FILE,
                self.data_queue, self.emg_rms_queue, self.imu1_angle_queue,
                self.imu2_angle_queue, self.imu1_predict_queue, self.pressure_queue
            )
            self.obs_thread.log_signal.connect(self.handle_log)
            self.obs_thread.start()

            if self.model is not None:
                self.predict_thread = PredictWorkerThread(self.imu1_predict_queue, self.shared_predict_results,
                                                          self.model, self.imu_mean, self.imu_std)
                self.predict_thread.start()
            self.running = True;
            self.run_button.setText("停止读取")
        else:
            ...
            self.running = False;
            self.run_button.setText("开始读取本地文件")
    ...
    def closeEvent(self, event):
        ...
        self.imu2_angle_plot.stop_timer()
        self.gait_time_series.stop_timer();
        self.gait_bar.stop_timer()
...
def main():
    ...
    font = QtGui.QFont("Microsoft YaHei", 8);
    app.setFont(font)
